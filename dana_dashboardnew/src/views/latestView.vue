<template>
  <v-app>
    ...
    >
      <template
        v-for="(col1, i) in filters1"
        v-slot:[`header.${i}`]="{ header }"
      >
        <div style="display: inline-block; padding: 0px 0" :key="col1">
          {{ header.text }}
        </div>
            ...
            <template v-slot:activator="{ on, attrs }">
              <v-btn color="indigo" icon v-bind="attrs" v-on="on">
                <v-icon
                  small
                  :color="
                    activeFilters1[header.value].length > 0 ? 'red' : 'default'
                  "
                >
                  mdi-filter-variant
                </v-icon>
...
export default {
  data() {
    return {
      ...
      items2: ["GTPR", "PCD", "PTGR"],
      items3: [1000, 2000, 3000, 4000, 5000, 6000, 4500, 5500],

      Application1: ["2Pinion", "4Pinion"],
  ...
  watch: {
    SortedGearPair(val) {
      ...
      // console.log("project : ",this.activeFilters1['Project'])
    },
  ...
  methods: {
    ...
    async save1() {
      ...
      console.log("JJ01 : ", this.postdata);
